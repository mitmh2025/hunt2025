...
function updateTasks(tasks) {
  ...
  for (var i = 0; i < tasks.length; i++) {
    ...
    const task = tasks[i];
    a.onclick = function () {
      markCompletion(task.text, !task.finished);
    };
    if (tasks[i].finished) {
...
function updateDisplay(instruction) {
  document.getElementById("verb").innerHTML = instruction.verb;
  document.getElementById("noun").innerHTML = instruction.noun;
...
function updateFromSocket(command, data) {
  ...
  switch (command) {
    case "game_state":
      ...
      updateDisplay(state["instruction"]);
      if (!displayOnly) {
...
function connectWebSocket(url, messageHandler) {
  ...
  ws.onclose = function (event) {
    ...
    setMessage(
      "Lost connnection to server. Please refresh the page.",
      "error",
      0
    );
  };
...
function setVideo(id) {
  if (!displayOnly) {
    document.getElementById("videoframe").src = id
