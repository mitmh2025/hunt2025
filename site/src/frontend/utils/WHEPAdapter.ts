// Forked from https://github.com/Eyevinn/webrtc-player/blob/main/src/adapters/WHEPAdapter.ts to improve
// reconnect behavior

import { type MediaConstraints } from "./WebRTCPlayer";
...
export class WHEPAdapter implements Adapter {
  ...
  private onIceCandidate(event: Event) {
    ...
    const candidateEvent = event as RTCPeerConnectionIceEvent;
  ...
  private onIceGatheringStateChange(_event: Event) {
    if (this.localPeer) {
      ...
      this.onDoneWaitingForCandidates().catch((error: unknown) => {
        this.error(
          "Error in onIceGatheringStateChange -> onDoneWaitingForCandidate",
          error,
        );
      });
    }
  ...
  private onIceGatheringTimeout() {
    ...
    this.onDoneWaitingForCandidates().catch((error: unknown) => {
      this.error(
        "Error in onIceGatheringTimeout -> onDoneWaitingForCandidate",
        error,
      );
    });
  }
  ...
  private async requestOffer() {
    ...
    throw new Error(`Unexpected WHEPType: ${this.whepType}`);
  }
  ...
  private async sendOffer() {
    ...
    if (this.whepType === WHEPType.Client && offer) {
      ...
      try {
        ...
        this.error("sendAnswer error", error);
        this.onErrorHandler("connectionfailed");
      }
    }
  }
