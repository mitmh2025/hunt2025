...
export const canUseKey = (level: number, room: Room) => {
  return (
    room.level === level &&
...
export type InternalizedStateType = {
  currentRoom: Room;
  compass?: number;
  key?: {
    level: number;
...
export function processCommand({
  ...
  startState: InternalizedStateType;
  command: string;
}): {
  message: string;
  haveKey: boolean;
  ...
  state: ExternalizedStateType;
} {
  const endState = tokenizeCommand(command).reduce<InternalizedStateType>(
    (state, command) => {
