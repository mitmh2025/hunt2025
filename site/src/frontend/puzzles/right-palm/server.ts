import { z } from "zod";
import { type PuzzleEndpoint } from "../types";
...
function inputsToOutputs(inputs: Inputs): Outputs {
  let prog = 0;
...
const handlers: PuzzleEndpoint[] = [
  {
    ...
    handler: (req, res) => {
      try {
        ...
        const result: { outputs: Outputs; additionalText?: string } = {
          outputs,
        };
        if (outputs[17]) {
